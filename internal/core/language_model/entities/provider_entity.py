...
class Provider(BaseModel):
    ...
    @model_validator(mode='after')
    @classmethod
    def validate_provider(cls, model: "Provider") -> "Provider":
        """服务提供者校验器，利用校验器完成该服务提供者的实体与类实例化"""

        provider_entity = model.provider_entity

        # 1. 动态导入模型类
        for model_type_enum in provider_entity.supported_model_types:
            model_type = model_type_enum.value  # 取枚举的字符串值 "chat" / "completion"
            symbol_name = model_type[0].upper() + model_type[1:]
            model.model_class_map[model_type] = dynamic_import(
                f"internal.core.language_model.providers.{provider_entity.name}.{model_type}",
                symbol_name
            )
        ...
        with open(positions_yaml_path, encoding="utf-8") as f:
            positions_yaml_data = yaml.safe_load(f) or []
        if not isinstance(positions_yaml_data, list):
            raise FailException("positions.yaml 数据格式错误")
        ...
        for model_name in positions_yaml_data:
            ...
            for parameter in yaml_parameters:
                ...
                if use_template:
                    ...
                    if default_parameter is None:
                        raise FailException(f"找不到参数模板：{use_template}")
                    del parameter["use_template"]
                    parameters.append({**default_parameter, **parameter})
                else:
                    parameters.append(parameter)
            ...
            model_yaml_data["parameters"] = parameters
            model.model_entity_map[model_name] = ModelEntity(**model_yaml_data)
