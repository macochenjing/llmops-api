# -*- coding: utf-8 -*-

"""
@Time   : 2024/12/30 10:48
@Author : chenjingmaco@gmail.com
@File   : test_app_handler.py
"""

import pytest
from pkg.response import HttpCode


# class TestAppHandler:
#     """app控制器的测试类"""
#
#     def test_completion(self, client):  # 在终端执行pytest的时候，会自动调用获取test/conftest/client
#         resp = client.post("/app/completion", json={"query": "你好，你是谁？"})
#         assert resp.status_code == 200
#         assert resp.json.get("code") == HttpCode.SUCCESS
#         print("响应内容: ", resp.json)

class TestAppHandler:
    """app控制器的测试类"""

    @pytest.mark.parametrize("query", [None, "你好，你是谁？"])
    def test_completion(self, query, client):  # 在终端执行pytest的时候，会自动调用获取test/conftest/client
        resp = client.post("/app/completion", json={"query": query})
        assert resp.status_code == 200

        if query is None:
            assert resp.json.get("code") == HttpCode.VALIDATE_ERROR
        else:
            assert resp.json.get("code") == HttpCode.SUCCESS

        # print("响应内容: ", resp.json)